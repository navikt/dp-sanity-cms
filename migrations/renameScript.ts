...
// Run this script with: `sanity exec --with-user-token migrations/renameScript.ts`
//
...
client.config({ dataset: "development" });
console.log(client.config());

const fetchDocuments = () =>
  client.fetch(`*[_type == 'oppsett' && defined(komIgangLenker)] {_id, _rev, komIgangLenker, sideoversiktLenker}`);
...
const buildPatches = (docs) =>
  docs.map((doc) => ({
    ...
    patch: {
      set: {
        snarveier: doc.komIgangLenker.map((it) => ({ ...it, _type: "snarvei" })),
        menyLenker: doc.sideoversiktLenker.map((it) =>
          it._type === "eksternLenke" ? { ...it, _type: "menylenkeEkstern" } : it
        ),
      },
      unset: ["komIgangLenker", "sideoversiktLenker"],
